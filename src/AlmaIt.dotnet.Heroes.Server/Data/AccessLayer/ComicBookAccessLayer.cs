namespace AlmaIt.Dotnet.Heroes.Server.Data.AccessLayer
{
    ...
    /// <summary>Class allowing access to <see cref="ComicBook" /> data object.</summary>
    internal class ComicBookAccessLayer : BaseAccessLayer<HeroesContext, ComicBook, int>, IComicBookAccessLayer
    {
        /// <summary>Initializes a new instance of the <see cref="ComicBookAccessLayer" /> class.</summary>
        ...
        /// <summary>Async method that return a ComicBook based on a id.</summary>
        /// <param name="id">Id of comic book to search.</param>
        /// <returns>Return the comic book correspoding to id.</returns>
        public override async Task<ComicBook> GetAsync(int id) => await this.GetAllAsync().SingleOrDefault(model => model.Id.CompareTo(id) == 0);
    }
}
